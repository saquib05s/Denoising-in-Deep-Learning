...
from utils import weights_init_kaiming, batch_psnr, init_logger, svd_orthogonalization, batch_ssim
...
def main(args):
    ...
    loader_train = DataLoader(
        dataset=dataset_train, num_workers=6, batch_size=args.batch_size, shuffle=True
    )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            stdn = np.random.uniform(
                args.noiseIntL[0], args.noiseIntL[1], size=noise.size()[0]
            )
            for nx in range(noise.size()[0]):
                sizen = noise[0, :, :, :].size()
                noise[nx, :, :, :] = torch.FloatTensor(sizen).normal_(
                    mean=0, std=stdn[nx]
                )
            ...
            # Results
            model.eval()
            out_train = torch.clamp(imgn_train - model(imgn_train, stdn_var), 0.0, 1.0)
        ...
        if training_params["step"] % args.save_every == 0:
            ...
            writer.add_scalar(
                "PSNR on training data", psnr_train, training_params["step"]
            )
            print(
    ...
    for valimg in dataset_val:
        ...
        noise = torch.FloatTensor(img_val.size()).normal_(
            mean=0, std=args.val_noiseL
        )
        imgn_val = img_val + noise
        img_val, imgn_val = Variable(img_val.cuda()), Variable(imgn_val.cuda())
        sigma_noise = Variable(torch.cuda.FloatTensor([args.val_noiseL]))
        out_val = torch.clamp(imgn_val - model(imgn_val, sigma_noise), 0.0, 1.0)
        psnr_val += batch_psnr(out_val, img_val, 1.0)
    psnr_val /= len(dataset_val)
    print("\n[epoch %d] PSNR_val: %.4f\n PSNR_val: %.4f" % (epoch + 1, psnr_val, ssim_val))
    ...
    try:
        if epoch == 0:
            ...
            for idx in range(2):
                imclean = utils.make_grid(
                    img_val.data[idx].clamp(0.0, 1.0),
                    nrow=2,
                    normalize=False,
                    scale_each=False,
                )
                imnsy = utils.make_grid(
                    imgn_val.data[idx].clamp(0.0, 1.0),
                    nrow=2,
                    normalize=False,
                    scale_each=False,
                )
        ...
        for idx in range(2):
            imrecons = utils.make_grid(
                out_val.data[idx].clamp(0.0, 1.0),
                nrow=2,
                normalize=False,
                scale_each=False,
            )
            writer.add_image(
                "Reconstructed validation image {}".format(idx), imrecons, epoch
            )
        # Log training images
        imclean = utils.make_grid(
            img_train.data, nrow=8, normalize=True, scale_each=True
        )
    ...
    if epoch % args.save_every_epochs == 0:
        torch.save(
            save_dict, os.path.join(args.log_dir, "ckpt_e{}.pth".format(epoch + 1))
        )
    del save_dict
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--epochs", "--e", type=int, default=80, help="Number of total training epochs"
    )
    parser.add_argument(
        "--resume_training",
        "--r",
    ...
    argspar.val_noiseL /= 255.0
    argspar.noiseIntL[0] /= 255.0
    argspar.noiseIntL[1] /= 255.0
