...
from utils import batch_psnr,ssim, normalize, batch_ssim, init_logger_ipol, \
...
def test_ffdnet(**args):
    ...
    for filename in os.listdir('x_test_data'):
        if rgb_den:
            in_ch = 3
            model_fn = 'models/net_rgb.pth'
            imorig = cv2.imread(os.path.join('/x_test_data', filename))
            ...
            imorig = cv2.imread(os.path.join('x_test_data', filename), cv2.IMREAD_GRAYSCALE)
        ...
        psnr1 += batch_psnr(outim, imorig, 1.)
        ssim1 += ssim(outim, imorig)

        # Compute difference
        diffout = 2 * (outim - imorig) + .5
        diffnoise = 2 * (imnoisy - imorig) + .5

    print('PSNR = ', psnr1, ' SSIM1 = ', ssim1)
# Save images
